...
class Card;
class Player;
//int ladders =0,snakes =1,cards=2 ; //representing the game obj with numbers 
class Grid
{
...
public:
	...
	void player0() ;                 // setting current player 0
	...
	Card * getnextcard ( const CellPosition & position);//                                 card
	//int OverlappingSnakes (CellPosition&start, CellPosition&end);
	int OverlappingLadders (CellPosition&start, CellPosition&end);
	...
	//====================saving===============
	void SaveAll(ofstream &OutFile,int type); //loops on each cell to call the save fn of the game objects in any cell
	void LoadAll(ifstream &inputfile );
	//===============

	int getCurrPlayerNumber(); //gets current player num

	void setcurrPlayerNumber(int num); //sets player num ???
